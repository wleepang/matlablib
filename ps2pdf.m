...
%      May 18, 2009: WLP added option for specifying a pdfmark file, but not sure how
%                    to incorporate it into the ghostcript command call.
...
function gsData = LocalParseArgs(varargin) 
    ...
    for i = 1 : 2 : length(varargin)-1 
        ...
        switch(lower(param_arg)) 
            ...
            % path to pdfMarks file
            case 'pdfmarkfile'
                if ~exist(param_value, 'file')
                    error('print:ghostscript', ...
                        'Can not find pdfmark file <%s> to include', ...
                        param_value)
                end
                gsData.pdfMarkFile = param_value;
